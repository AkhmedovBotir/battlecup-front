...
export default function MatchDetails() {
  ...
  const [match, setMatch] = useState(matchData[0]);

  useSEO({
    title: match ? `${match.teams[0].name} vs ${match.teams[1].name} - O'yin ${matchId}` : "O'yin tafsilotlari",
    description: match ? 
      `${match.teams[0].name} vs ${match.teams[1].name} o'yini, ${match.mode} turniri doirasida. O'yin davomiyligi: ${match.duration}, g'olib: ${match.teams[0].result}` :
      "BattleCup platformasidagi Dota 2 o'yini tafsilotlari",
    keywords: `dota 2, match, ${match?.teams[0].name}, ${match?.teams[1].name}, ${match?.mode}, battlecup`,
    image: match?.preview_image,
    type: "article",
    publishedTime: match?.date,
    modifiedTime: match?.date,
    section: "matches",
    canonicalPath: `/matches/${matchId}/overview`
  });
            ...
            <div className='flex items-center justify-between gap-8 w-3/4'>
              <div className='flex flex-row justify-between items-center w-1/3'>
                ...
                <div className='text-[#20C867] flex flex-row justify-end w-full'>
                  <div className='text-[24px] font-bold'>{match.teams[0].name}</div>
                  <div className='text-[36px] font-bold leading-8 ml-5'>{match.teams[0].score}</div>
                </div>
              </div>
              <div className='text-center w-1/4 bg-[#13161C] h-full py-[17px]'>
                <div className='text-[#BCC3D0] text-[16px]'>{match.mode}</div>
                <div className='text-[#BCC3D0] text-[16px] font-bold my-1'>{match.duration}</div>
                <div className='text-[#59606F] text-[16px]'>{match.date}</div>
              </div>
              <div className='w-2/5 px-5 py-4'>
                <div className='text-[#F23B3B] flex flex-row justify-start w-full'>
                  <div className='text-[36px] font-bold leading-8 mr-5'>{match.teams[1].score}</div>
                  <div className='text-[24px] font-bold'>{match.teams[1].name}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`my-6 border border-[#181C25] bg-[#12151B] rounded-[5px] ${isChatPage ? 'max-w-[1064px] mx-auto' : ''}`}>
          <div className='flex justify-between bg-[#12151B]'>
            ...
            <div className='flex gap-4 p-[12px]'>
              ...
              <button className='border border-[#333B4E] px-2 rounded-lg text-[#8993A6] hover:text-white hover:border-[#fff] transition-colors flex items-center gap-2 text-sm'>
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M11.0003 3.66699V14.667M11.0003 14.667L15.5837 10.084M11.0003 14.667L6.41699 10.084" stroke="#BCC3D0" stroke-width="1.66" stroke-linecap="round" stroke-linejoin="round"/>
